...
func TestBasicAuthMissingAuthHeader(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/sensor/0", nil)
...
func TestBasicAuthErrorOnSplitDecodedString(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/sensor/0", nil)
	req.Header.Add("Authorization", "Basic GVsaW9zOnByb3RvdHlwZQ==")
...
func TestBasicAuthInvalidCredentials(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/sensor/0", nil)
	req.Header.Add("Authorization", "Basic aGVsaW9zOnBhc3N3b3Jk")
